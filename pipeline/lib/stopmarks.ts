 ...
 *
 * The band ordinals are the same centred offsets `build.ts` gives the route
...
export interface MarkBundle {
  /** Line ids in draw order - index `i` is offset ordinal `i - (n-1)/2`. */
  lineIds: string[];
