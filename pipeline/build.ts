...
import {
  MODE_SPECS, STOP_TIER_BY_RANK, fallbackColour, normaliseColour, stopRank, tileBufferUnits,
  type Mode,
} from '../shared/lnvg.ts';
import {
...
async function main() {
  ...
  /** The widest slot emitted, which is what the tile buffer is cut for below. */
  let maxAbsOffset = 0;
  segInfos.forEach((seg, segIdx) => {
  ...
  for (const { lineId, bundle, minzoom, step } of staircases) {
    const line = lines.get(lineId)!;
    maxAbsOffset = Math.max(maxAbsOffset, Math.abs(step.offset));
    features.push({
      type: 'Feature',
  ...
  // How wide to cut the tiles.
  //
  // A band is drawn where its slot puts it, not where its geometry lies, and
  // MapLibre applies that offset only after clipping the tile's lines to the
  // tile's own square - so a tile that does not carry the geometry a band was
  // offset from cannot draw that band, and the line goes missing from the tile
  // edge until its own geometry is clear of it. See `bandReachPx` in
  // shared/lnvg.ts. Sized off the widest slot this build actually emitted
  // rather than off a constant, because that is what varies: a regional
  // extract fans four bands where the national one fans twenty-three, and
  // there is no reason for the small build to pay the big build's margin.
  const buffer = tileBufferUnits(maxAbsOffset);
  writeFileSync(`${OUT}/tile-buffer`, `${buffer}\n`);
  console.log(
    `==> widest band sits ${maxAbsOffset} pitches off its alignment `
    + `-> tile buffer ${buffer}`,
  );
