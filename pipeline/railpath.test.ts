...
import {
  buildRailGraph, nearestNode, routeBetween, metres, type Coord, type RailWay,
...
const at = (eastM: number, northM: number): Coord =>
  [9.7 + eastM * M_LON, 52.4 + northM * M_LAT];
...
test('joins a way that ends partway along another, unsplit way', () => {
  ...
  const graph = buildRailGraph([
    way('main', [[0, 0], [1000, 0], [2000, 0]]),
    way('branch', [[1000, 0], [1000, 1200]]),
  ]);
...
test('prefers the line the closure names over a shorter parallel one', () => {
  ...
  const ways = [
    way('west-a', [[0, 0], [0, 900]], { ref: '1700' }),
    way('west-b', [[0, 900], [2000, 900]], { ref: '1700' }),
    way('west-c', [[2000, 900], [2000, 0]], { ref: '1700' }),
    way('direct', [[0, 0], [2000, 0]], { ref: '2200' }),
  ];
...
test('an untagged way is not treated as being off the line', () => {
  ...
  const graph = buildRailGraph([
    way('gap', [[0, 0], [1000, 0]]),
    way('tagged', [[1000, 0], [2000, 0]], { ref: '1700' }),
    way('detour-a', [[0, 0], [0, 400]], { ref: '1700' }),
    way('detour-b', [[0, 400], [1000, 400]], { ref: '1700' }),
    way('detour-c', [[1000, 400], [1000, 0]], { ref: '1700' }),
  ]);
...
test('takes the running line rather than a shortcut through a yard', () => {
  const graph = buildRailGraph([
    way('running-a', [[0, 0], [1000, 600]]),
    way('running-b', [[1000, 600], [2000, 0]]),
    way('yard', [[0, 0], [2000, 0]], { service: 'yard' }),
  ]);
...
test('trims the path back to the operating points the closure names', () => {
  ...
  const graph = buildRailGraph([
    ...
    way('b', [[1000, 0], [2000, 0]]),
    way('c', [[2000, 0], [3000, 0]]),
    way('branch-1', [[1000, 0], [1000, 800]]),
    way('branch-2', [[2000, 0], [2000, 800]]),
  ]);
...
test('a closed loop contributes no self-edge', () => {
  const graph = buildRailGraph([
    way('loop', [[0, 0], [500, 500], [1000, 0], [0, 0]]),
  ]);
